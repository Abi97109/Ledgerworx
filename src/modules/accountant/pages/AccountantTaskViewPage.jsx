...
function AccountantTaskViewPage() {
  ...
  useEffect(() => {
    ...
    async function loadLiveTaskDetails() {
      try {
        const response = await fetch(
          buildLegacyUrl(`${ACCOUNTANT_LEGACY_PATHS.zohoApi}?action=task_detail&id=${encodeURIComponent(taskIdParam)}`),
        );
        ...
        if (data && data.success && data.data) {
          setTaskDetail(normalizeTaskDetailPayload(data.data, taskIdParam));
          setSourceMessage(data.source === "zoho" ? ACCOUNTANT_TASK_VIEW_SOURCE_TEXT.zoho : ACCOUNTANT_TASK_VIEW_SOURCE_TEXT.fallback);
          return;
        }

        setSourceMessage(ACCOUNTANT_TASK_VIEW_SOURCE_TEXT.loadFailed);
      } catch (error) {
        if (!isMounted) {
          return;
        }

        setSourceMessage(ACCOUNTANT_TASK_VIEW_SOURCE_TEXT.connectionFailed);
      }
    }
  ...
  const handleCompletionSubmit = useCallback(
    async (event) => {
      ...
      try {
        ...
        const response = await fetch(buildLegacyUrl(ACCOUNTANT_LEGACY_PATHS.taskCompletePortal), {
          method: "POST",
          body: formData,
        });
        ...
        if (!response.ok || !payload.success) {
          throw new Error(payload.message || "Portal sync failed.");
        }

        setCompletionMessage({
          type: "success",
          text: payload.message || "Task completed and synced to portal.",
        });
  ...
  const buildTaskDocumentUrl = useCallback((documentPath) => {
    ...
    if (rawPath.startsWith("/") || rawPath.startsWith("accountant-n/")) {
      return buildLegacyUrl(rawPath);
    }

    return buildLegacyUrl(`accountant-n/php/${rawPath}`);
  }, []);
