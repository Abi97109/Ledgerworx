...
function AccountantInvoicesPage() {
  ...
  const fetchInvoicesFromZoho = useCallback(async () => {
    try {
      const response = await fetch(buildLegacyUrl(`${ACCOUNTANT_LEGACY_PATHS.zohoApi}?action=invoices`));
      const payload = await response.json();

      if (payload && payload.success && Array.isArray(payload.data) && payload.data.length > 0) {
        setAllInvoices(normalizeInvoiceList(payload.data, getVerifiedInvoiceMap()));
