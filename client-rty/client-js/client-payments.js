        document.addEventListener('DOMContentLoaded', function() {
            ...
            function redirectToPaymentGateway(data, source) {
                const packageName = data && data.title ? data.title : 'Service Payment';
                const packagePrice = data && data.amount ? data.amount : 'Amount will be confirmed';
                const requestId = data && data.requestId ? data.requestId : 'N/A';
            ...
            paymentDetailPayNow && paymentDetailPayNow.addEventListener('click', () => {
                ...
                openModal('Redirect To Payment Gateway', `You are being redirected to payment gateway for ${title} (${amount}). Continue?`, () => {
                    redirectToPaymentGateway(selectedPaymentDetails, 'payments-detail-modal');
                });
            });
                ...
                button.addEventListener('click', () => {
                    ...
                    if (label === 'pay now' || label === 'retry payment') {
                        openModal('Redirect To Payment Gateway', `You are being redirected to payment gateway for ${data.title} (${data.amount}). Continue?`, () => {
                            redirectToPaymentGateway(data, 'payments-item-tile');
                        });
                        return;
                    }
            ...
            paymentCardButtons.forEach((button) => {
                button.addEventListener('click', () => {
                    ...
                    if (label === 'pay now') {
                        openModal('Redirect To Payment Gateway', `You are being redirected to payment gateway for ${data.title} (${data.amount}). Continue?`, () => {
                            redirectToPaymentGateway(data, 'payments-summary-tile');
                        });
                        return;
                    }
