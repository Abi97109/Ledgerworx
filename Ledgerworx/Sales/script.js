(function () {
  ...
  function createLeadsModule() {
    ...
    function getFilteredLeads() {
      ...
      if (query) {
        filtered = filtered.filter((lead) => {
          return lead.name.toLowerCase().includes(query) || lead.contact.toLowerCase().includes(query);
        });
      }
    ...
    function render(leads) {
      ...
      leads.forEach((lead, index) => {
        table.innerHTML +=
          ...
          '<small class="text-muted-small">' + lead.contact + "</small>" +
          "</div>" +
          "</div>" +
          "</td>" +
          '<td><span class="lw-badge ' + lead.status + '">' + capitalize(lead.status) + "</span></td>" +
          '<td class="text-muted-small">' + lead.followUp + "</td>" +
          '<td class="text-owner">' + lead.owner + "</td>" +
          '<td><button class="lw-btn lw-btn--small" data-action="open-convert-modal" data-id="' + lead.id + '">Convert to Client</button></td>' +
          '<td><button class="lw-btn lw-btn--small" data-action="send-invoice" data-id="' + lead.id + '">Send Invoice</button></td>' +
    ...
    function addLead() {
      ...
      const status = statusInput ? statusInput.value : "warm";
  ...
  function createLeadDetailModule() {
    ...
    function populateLead() {
      ...
      if (leadPhone) leadPhone.textContent = selectedLead.contact;
