...
function lw_cache_payload_collection_count( $payload ) {
	if ( ! is_array( $payload ) ) {
		return 0;
	}

	foreach ( array( 'packages', 'services', 'categories', 'invoices' ) as $key ) {
		if ( array_key_exists( $key, $payload ) ) {
			return is_array( $payload[ $key ] ) ? count( $payload[ $key ] ) : 0;
		}
	}
...
function lw_cache_should_preserve_previous_payload( $fresh_payload, $cached_payload ) {
	if ( ! is_array( $fresh_payload ) || ! is_array( $cached_payload ) ) {
		return false;
	}

	$keys = array( 'packages', 'services', 'categories', 'invoices' );

	foreach ( $keys as $key ) {
		$fresh_has_key  = array_key_exists( $key, $fresh_payload );
		$cached_has_key = array_key_exists( $key, $cached_payload );

		if ( ! $fresh_has_key || ! $cached_has_key ) {
			continue;
		}

		$fresh_count  = is_array( $fresh_payload[ $key ] ) ? count( $fresh_payload[ $key ] ) : 0;
		$cached_count = is_array( $cached_payload[ $key ] ) ? count( $cached_payload[ $key ] ) : 0;

		if ( 0 === $fresh_count && $cached_count > 0 ) {
			return true;
		}
	}
...
function lw_cache_remember( $cache_key, $ttl, $callback, $scope = 'global' ) {
	...
	try {
		...
		if ( $cached && ! empty( $cached['payload'] ) && lw_cache_should_preserve_previous_payload( $fresh_payload, $cached['payload'] ) ) {
			lw_write_cache_row(
				$cache_key,
				$scope,
				$cached['payload'],
				...
				'stale',
				'',
				'Empty refresh payload received from upstream; preserved previous cache.'
			);
			...
			return array(
				...
				'meta' => array(
					...
					'stale'       => true,
					'syncStatus'  => 'stale',
					'error'       => 'Empty refresh payload received from upstream; preserved previous cache.',
				),
			);
		}
