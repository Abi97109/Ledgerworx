...
function AccountantTasksPage() {
  ...
  const fetchTasksFromZoho = useCallback(async () => {
    try {
      const response = await fetch(buildLegacyUrl(`${ACCOUNTANT_LEGACY_PATHS.zohoApi}?action=tasks`));
      const data = await response.json();

      if (data && data.success && Array.isArray(data.data) && data.data.length > 0) {
        setAllTasks(normalizeTasksList(data.data));
        setSyncStatusText(data.source === "zoho" ? "Live Zoho Data" : "Fallback Mode (Zoho Unavailable)");
        return;
      }
    } catch (error) {
      // Keep tasks page usable and continue with demo data mode.
    }
